...
class TestGetTablenameForeignKeys:
    ...
    TESTS = [
        ...
        pytest.param(
            ...
            {"table 1": {"foreign1.key1", "foreign1.key2"}},
        ...
        pytest.param(
            ...
            {"table 1": {"foreign1.key1", "foreign2.key1"}},
            id="single table multiple schema single foreign key",
        ),
        pytest.param(
            ...
            {"table 1": {"foreign1.key1"}, "table 2": {"foreign2.key1"}},
            id="multiple table single schema single foreign key",
        ),
    ]

    @staticmethod
    @pytest.mark.parametrize("tablename_names, schemas, expected_mapping", TESTS)
    @pytest.mark.schemas
    @pytest.mark.association
    def test_(tablename_names, schemas, expected_mapping):
        """
        ...
        returned_mapping = association._get_tablename_foreign_keys(
            tablename_names=tablename_names, schemas=schemas
        )
