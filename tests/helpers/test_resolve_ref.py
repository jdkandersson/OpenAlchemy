...
@pytest.mark.helper
def test_legacy_resolve_ref_exists():
    """
    GIVEN
    WHEN
    THEN helpers has legacy_resolve_ref property.
    """
    assert hasattr(helpers, "legacy_resolve_ref")


def test_legacy_resolve_ref_not_ref_schema():
    """
    GIVEN schema that does not have $ref
    WHEN legacy_resolve_ref is called with the schema
    THEN the schema is returned.
    """
    schema = types.Schema("Schema", {"type": "integer"})
    schemas = {}

    return_schema = helpers.legacy_resolve_ref(schema=schema, schemas=schemas)

    expected_schema = types.Schema("Schema", {"type": "integer"})
    assert return_schema == expected_schema


def test_legacy_resolve_ref_not_schema():
    """
    ...
    with pytest.raises(exceptions.SchemaNotFoundError):
        helpers.legacy_resolve_ref(schema=schema, schemas=schemas)


def test_legacy_resolve_ref_not_defined():
    """
...
def test_legacy_resolve_ref_single():
    """
    GIVEN schema that references another schema and schemas
    WHEN legacy_resolve_ref is called with the schema and schemas
...
def test_legacy_resolve_ref_nested():
    """
